...
class ConstructionIndustrySchemeConnector @Inject() (config: ServicesConfig, http: HttpClientV2)(implicit
  ...
  def getSubmissionStatus(pollUrl: String, submissionId: String)(implicit
    hc: HeaderCarrier
  ): Future[ChrisPollResponse] =
    http
      .get(url"$cisBaseUrl/submissions/poll?submissionId=$submissionId&pollUrl=$pollUrl")
  ...
  def deleteAllMonthlyReturnItems(
    payload: DeleteAllMonthlyReturnItemsRequest
  )(implicit hc: HeaderCarrier): Future[Unit] =
    http
