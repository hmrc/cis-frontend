...
import viewmodels.checkAnswers.monthlyreturns.EmploymentStatusDeclarationSummary

class EmploymentStatusDeclarationSummarySpec extends SpecBase with OptionValues {

  private implicit val messages: Messages = Helpers.stubMessages()

  "EmploymentStatusDeclarationSummary" - {
    ...
    "when answer is present" - {
      ...
      "must return a SummaryListRow with 'Yes' when answer is true" in {
        ...
        val result = EmploymentStatusDeclarationSummary.row(answers).value

        result.key.content.asHtml.toString   must include(
          messages("monthlyreturns.employmentStatusDeclaration.checkYourAnswersLabel")
        )
      ...
      "must return a SummaryListRow with 'No' when answer is false" in {
        val answers = emptyUserAnswers.set(EmploymentStatusDeclarationPage, false).success.value
    ...
    "when answer is not present" - {
      ...
      "must return None" in {
        ...
        val result = EmploymentStatusDeclarationSummary.row(answers)
