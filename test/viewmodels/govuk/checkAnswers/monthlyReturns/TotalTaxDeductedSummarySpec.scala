...
class TotalTaxDeductedSummarySpec extends AnyFreeSpec with Matchers with OptionValues {

  private implicit val messages: Messages = Helpers.stubMessages()

  "TotalTaxDeductedSummary" - {
    ...
    "row" - {
      ...
      "must include the correct change action URL" in {
        ...
        action.href mustEqual controllers.monthlyreturns.routes.TotalTaxDeductedController.onPageLoad(CheckMode, 1, None).url
      }
      ...
      "must format different BigDecimal values correctly" in {
        val testCases = Seq(
          BigDecimal(0)          -> "0",
          BigDecimal(100.50)     -> "100.5",
          BigDecimal(1000000.99) -> "1000000.99"
        )
