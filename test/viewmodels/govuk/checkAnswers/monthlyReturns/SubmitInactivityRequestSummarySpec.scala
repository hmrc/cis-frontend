...
class SubmitInactivityRequestSummarySpec extends SpecBase with OptionValues {
  ...
  "SubmitInactivityRequestSummary" - {
    ...
    "when answer is present" - {
      ...
      "must return a SummaryListRow with 'Yes' when answer is true" in {
        ...
        result.actions.value.items.head.attributes mustBe Map("id" -> "change-submit-inactivity-request")
      }
