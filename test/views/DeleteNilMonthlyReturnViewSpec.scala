...
class DeleteNilMonthlyReturnViewSpec extends SpecBase {
  ...
  "DeleteNilMonthlyReturnView" - {
    "must render the page with the correct html elements" in new Setup {
      ...
      doc.title             must include(messages("monthlyreturns.deleteNilMonthlyReturn.title"))
      doc.select("h1").text must include(messages("monthlyreturns.deleteNilMonthlyReturn.heading"))
  ...
  trait Setup {
    ...
    val view                                      = app.injector.instanceOf[DeleteNilMonthlyReturnView]
