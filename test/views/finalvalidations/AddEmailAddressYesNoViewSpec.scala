...
class AddEmailAddressYesNoViewSpec extends SpecBase {

  "AddEmailAddressYesNoView" - {
    "must render the page with the correct html elements" in new Setup {
      val doc: Document = Jsoup.parse(html.toString)
      doc.title             must include(messages("finalvalidations.addEmailAddressYesNo.title"))
      doc.select("h1").text must include(messages("finalvalidations.addEmailAddressYesNo.heading"))
  ...
  trait Setup {
    ...
    val view                                      = app.injector.instanceOf[AddEmailAddressYesNoView]
