...
class DeleteMonthlyReturnViewSpec extends SpecBase {
  ...
  "DeleteMonthlyReturnView" - {
    "must render the page with the correct html elements" in new Setup {
      ...
      doc.title             must include(messages("monthlyreturns.deleteMonthlyReturn.title"))
      doc.select("h1").text must include(messages("monthlyreturns.deleteMonthlyReturn.heading"))
  ...
  trait Setup {
    ...
    val view                                      = app.injector.instanceOf[DeleteMonthlyReturnView]
