...
class DeleteMonthlyReturnViewSpec extends SpecBase {
  ...
  "DeleteMonthlyReturnView" - {
    ...
    "must render the page with the correct html elements (English) including monthYear" in new Setup {
      ...
      doc.title must include(messagesEn("monthlyreturns.deleteMonthlyReturn.title", monthYearEn))
      doc.title must include(monthYearEn)

      doc.select("h1").text must include(messagesEn("monthlyreturns.deleteMonthlyReturn.heading", monthYearEn))
    ...
    "must render the page with the correct html elements (Welsh) including monthYear" in new Setup {
      ...
      doc.title must include(messagesCy("monthlyreturns.deleteMonthlyReturn.title", monthYearCy))
      doc.title must include(monthYearCy)

      doc.select("h1").text must include(messagesCy("monthlyreturns.deleteMonthlyReturn.heading", monthYearCy))
