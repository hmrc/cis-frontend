...
class PaymentDetailsConfirmationViewSpec extends SpecBase {

  "PaymentDetailsConfirmationView" - {
    "must render the page with the correct html elements" in new Setup {
      val doc: Document = Jsoup.parse(html.toString)
      doc.title             must include(messages("monthlyreturns.paymentDetailsConfirmation.title"))
      doc.select("h1").text must include(messages("monthlyreturns.paymentDetailsConfirmation.heading"))

      doc.getElementsByClass("govuk-hint").text   must include(messages("monthlyreturns.paymentDetailsConfirmation.hint"))
    ...
    "must render radio buttons with correct values" in new Setup {
      ...
      doc.select("label[for=value_0]").text() must include(messages("monthlyreturns.paymentDetailsConfirmation.yes"))
      doc.select("label[for=value_1]").text() must include(messages("monthlyreturns.paymentDetailsConfirmation.no"))
    }
  ...
  trait Setup {
    ...
    val view                                      = app.injector.instanceOf[PaymentDetailsConfirmationView]
