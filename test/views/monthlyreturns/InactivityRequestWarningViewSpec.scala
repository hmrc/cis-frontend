...
class InactivityRequestWarningViewSpec extends SpecBase {
  ...
  "InactivityRequestWarningView" - {
    "must render the content on the page" in new Setup {
      ...
      doc.select("button").text must include(messages("monthlyreturns.inactivityRequestWarning.submit"))
    }
  }

  trait Setup {
    val app: Application                          = applicationBuilder().build()
    val view: InactivityRequestWarningView        = app.injector.instanceOf[InactivityRequestWarningView]
