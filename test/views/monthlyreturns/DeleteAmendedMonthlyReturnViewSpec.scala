...
import forms.monthlyreturns.DeleteAmendedMonthlyReturnFormProvider
...
import views.html.monthlyreturns.DeleteAmendedMonthlyReturnView

import java.time.LocalDate
import java.time.format.DateTimeFormatter
import java.util.Locale

class DeleteAmendedMonthlyReturnViewSpec extends SpecBase {

  "DeleteAmendedMonthlyReturnView" - {
    ...
    "must render the page with the correct html elements (English) including monthYear" in new Setup {
      ...
      doc.title must include(messagesEn("monthlyreturns.deleteAmendedMonthlyReturn.title", monthYearEn))
      doc.title must include(monthYearEn)

      doc.select("h1").text must include(messagesEn("monthlyreturns.deleteAmendedMonthlyReturn.heading", monthYearEn))
    ...
    "must render the page with the correct html elements (Welsh) including monthYear" in new Setup {
      ...
      doc.title must include(messagesCy("monthlyreturns.deleteAmendedMonthlyReturn.title", monthYearCy))
      doc.title must include(monthYearCy)

      doc.select("h1").text must include(messagesCy("monthlyreturns.deleteAmendedMonthlyReturn.heading", monthYearCy))
  ...
  trait Setup {
    ...
    val view: DeleteAmendedMonthlyReturnView      = app.injector.instanceOf[DeleteAmendedMonthlyReturnView]
