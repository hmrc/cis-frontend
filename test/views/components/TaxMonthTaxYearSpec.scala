...
class TaxMonthTaxYearSpec extends PlaySpec with GuiceOneAppPerSuite {
  ...
  "TaxMonthTaxYear component" should {
    ...
    "render legend when legendContent is provided" in new Setup {
      val formWithValues: Form[TestModel] = form.bind(
        Map(
          "taxMonth" -> "3",
          "taxYear"  -> "2024"
      ...
      val output: HtmlFormat.Appendable = taxMonthTaxYearView(
        ...
        monthLabel = monthLabel,
        yearLabel = yearLabel,
        legendContent = Some("Tax period")
      )(messages)

      val html: Document = Jsoup.parse(output.toString)

      html.getElementsByTag("legend").text() must include("Tax period")
    }

    "not render legend when legendContent is None" in new Setup {
      val formWithValues: Form[TestModel] = form.bind(
        Map(
          "taxMonth" -> "3",
          "taxYear"  -> "2024"
      ...
      html.getElementsByClass("govuk-fieldset__legend").size() mustBe 0
    }

    "render legend as page heading when legendAsPageHeading is true" in new Setup {
      val formWithValues: Form[TestModel] = form.bind(
        Map(
          "taxMonth" -> "3",
          "taxYear"  -> "2024"
      ...
      html.getElementsByClass("govuk-fieldset__heading").size() mustBe 1
      html.getElementsByTag("h1").size() mustBe 1
    }
