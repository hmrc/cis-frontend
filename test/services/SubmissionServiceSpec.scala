...
import pages.submission.{PollIntervalPage, PollUrlPage, SubmissionDetailsPage}
...
class SubmissionServiceSpec extends SpecBase with TryValues {
  ...
  "checkAndUpdateSubmissionStatus" - {
    "fail when SubmissionDetailsPage is not present" in {
      ...
      when(connector.getSubmissionStatus(any))
    ...
    "update status and save to repository when submission is not timed out" in {
      ...
      val ua = uaBase
        ...
        .success
        .value
      when(connector.getSubmissionStatus(eqTo(submittedAt)))
        .thenReturn(Future.successful("SUBMITTED"))
      when(sessionRepository.set(any[UserAnswers]))
        .thenReturn(Future.successful(true))
      ...
      verify(connector).getSubmissionStatus(eqTo(submittedAt))
