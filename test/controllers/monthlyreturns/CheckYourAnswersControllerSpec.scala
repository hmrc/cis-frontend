...
import viewmodels.checkAnswers.monthlyreturns.{ConfirmationByEmailSummary, DateConfirmNilPaymentsSummary, DateConfirmPaymentsSummary, EmploymentStatusDeclarationSummary, EnterYourEmailAddressSummary, PaymentsToSubcontractorsSummary, ReturnTypeSummary, SubmitInactivityRequestSummary}
import views.html.monthlyreturns.CheckYourAnswersView
import org.scalatestplus.mockito.MockitoSugar
import pages.monthlyreturns.{CisIdPage, ConfirmationByEmailPage, DateConfirmNilPaymentsPage, DateConfirmPaymentsPage, EmploymentStatusDeclarationPage, EnterYourEmailAddressPage, NilReturnStatusPage, SubmitInactivityRequestPage}
...
class CheckYourAnswersControllerSpec extends SpecBase with SummaryListFluency with MockitoSugar {
  ...
  "Check Your Answers Controller" - {
    ...
    "must include SubmitInactivityRequest row in nil return CYA when page data is set" in {

      val userAnswers = userAnswersWithCisId
        .set(DateConfirmNilPaymentsPage, LocalDate.of(2025, 3, 1))
        .success
        .value
      ...
      running(application) {
        ...
        val returnDetailsList = SummaryListViewModel(
          Seq(
            ...
            DateConfirmNilPaymentsSummary.row(userAnswers)(messages(application)).get,
            PaymentsToSubcontractorsSummary.row(messages(application)).get,
            SubmitInactivityRequestSummary.row(userAnswers)(messages(application)).get
          )
        )
        val emailList         = SummaryListViewModel(Seq.empty)
