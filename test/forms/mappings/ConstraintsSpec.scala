...
class ConstraintsSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with Generators with Constraints {
  ...
  "maximumCurrency" - {
    ...
    "must return Invalid without formatted value when includeFormattedValue is false" in {
      val result = maximumCurrency(1, "error.max", includeFormattedValue = false).apply(1.01)
      result mustEqual Invalid("error.max")
    }

    "must return Invalid with formatted value when includeFormattedValue is true" in {
      val result = maximumCurrency(1, "error.max", includeFormattedValue = true).apply(1.01)
      result mustEqual Invalid("error.max", CurrencyFormatter.currencyFormat(1))
    }
  }
  ...
  "nonEmptySet" - {
    ...
    "must return Valid for a set with a single element" in {
      val result = nonEmptySet("error.empty").apply(Set(1))
      result mustEqual Valid
    }

    "must return Invalid for an empty set" in {
      val result = nonEmptySet("error.empty").apply(Set.empty[Int])
